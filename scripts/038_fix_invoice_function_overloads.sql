-- =====================================================
-- FIX INVOICE FUNCTION OVERLOADS AND INCONSISTENCIES
-- =====================================================
-- This script fixes the function overload resolution issues
-- and ensures consistent function naming and parameter types
...
-- =====================================================
-- 4. ENSURE INVOICE SEQUENCES TABLE EXISTS WITH CORRECT DATA
-- =====================================================
...
-- =====================================================
-- 5. VERIFY FUNCTIONS WORK CORRECTLY
-- =====================================================

-- Test get_next_invoice_number function
DO $$
DECLARE
  test_number TEXT;
BEGIN
  -- Test with different invoice types
  SELECT public.get_next_invoice_number('BASICA') INTO test_number;
  RAISE NOTICE 'SUCCESS: get_next_invoice_number(BASICA) = %', test_number;
  
  SELECT public.get_next_invoice_number('VALOR_FISCAL') INTO test_number;
  RAISE NOTICE 'SUCCESS: get_next_invoice_number(VALOR_FISCAL) = %', test_number;
  
  SELECT public.get_next_invoice_number('VALOR_GUBERNAMENTAL') INTO test_number;
  RAISE NOTICE 'SUCCESS: get_next_invoice_number(VALOR_GUBERNAMENTAL) = %', test_number;
  
EXCEPTION
  WHEN OTHERS THEN
    RAISE NOTICE 'ERROR testing get_next_invoice_number: %', SQLERRM;
END $$;
...
SELECT '=== FUNCTION OVERLOAD FIX SUMMARY ===' as section;

SELECT 
    'get_next_invoice_number(TEXT)' as function_name,
    CASE WHEN EXISTS (SELECT 1 FROM information_schema.routines 
                      WHERE routine_name = 'get_next_invoice_number' 
                      AND routine_schema = 'public'
                      AND specific_name LIKE '%TEXT%') 
         THEN 'CREATED' 
         ELSE 'ERROR' 
    END as status
UNION ALL
SELECT 
    'convert_order_to_invoice(UUID, TEXT)' as function_name,
    CASE WHEN EXISTS (SELECT 1 FROM information_schema.routines 
...
UNION ALL
SELECT 
    'invoice_sequences table' as function_name,
...
SELECT 'Function overload issues fixed. All functions now use consistent TEXT parameters.' as result;
